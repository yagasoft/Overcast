 ...
 * 
 *			Modified: Apr 15, 2014 (8:04:17 AM)
...
public abstract class Folder<T> extends Container<T>
{
	...
	{
		Logger.newTitledSection("create folder from path");
		Logger.newEntry("folder: " + parentPath + "/" + name);
		...
		while (splitPath.size() > 0)
		{
			...
			Logger.newEntry("created mid folder: " + parent.path);
		}
		...
		if (result != null)
		{
			Logger.newEntry("problem with creating nodes to reach desired folder: " + parentPath + "/" + name);
	...
	public void add(Folder<?> folder)
	{
		...
		Logger.newSection("added folder " + folder.path + " to parent " + path);
	}
	...
	public void add(File<?> file)
	{
		...
		Logger.newSection("added file " + file.path + " to parent " + path);
	}
	...
	public void remove(Folder<?> folder)
	{
		...
		Logger.newSection("removed folder " + folder.path + " from parent " + path);
	}
	...
	public void remove(File<?> file)
	{
		...
		Logger.newSection("removed file " + file.path + " from parent " + path);
	}
	...
	public void remove(String id)
	{
		...
		Logger.newSection("removed file/folder: " + id);
	}
	...
	{
		Logger.newTitledSection("searching by id");
		Logger.newEntry("searching " + id + " in " + path);
		...
		{	// else, if recursive search is wanted, check sub-folders.
			for (Folder<?> folder : folders.values())	// get the sub-folders from the list.
			{
				...
				if (container != null)
				{
					Logger.newEntry("found " + container.path + " in " + path);
					return (S) container;
				}
			}
		}
	...
	{
		Logger.newTitledSection("searching by name: " + name);
		Logger.newEntry("searching " + name + " in " + path);
	...
	{
		Logger.newTitledSection("removing obsolete containers");
		...
		for (String container : containers)
		{
			...
			if ( !ids.contains(container))
			{
				...
				Logger.newEntry("removed obsolete: " + container);
			}
		}
		...
		if (filter)
		{
			...
			for (String container : containers)
			{
				...
				Logger.newEntry("filtering existing: " + container);
			}
		}
	}
	...
	{
		Logger.newTitledSection("removing obsolete folders");
		...
		for (String folder : foldersList)
		{
			if ( !folderIds.contains(folder))
			{
				...
				Logger.newEntry("removed obsolete: " + folder);
			}
		}
		...
		if (filter)
		{
			for (String folder : foldersList)
			{
				...
				Logger.newEntry("filtering existing: " + folder);
			}
		}
	}
	...
	{
		Logger.newTitledSection("removing obsolete files");
		...
		for (String file : filesList)
		{
			if ( !fileIds.contains(file))
			{
				...
				Logger.newEntry("removed obsolete: " + file);
			}
		}
		...
		if (filter)
		{
			for (String file : filesList)
			{
				...
				Logger.newEntry("filtering existing: " + file);
			}
		}
	}
